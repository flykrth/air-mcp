...
class SupplierIntelligenceAgent(BaseAgent):
    ...
    def execute(self, state: Dict[str, Any], mcp_client: Any) -> Dict[str, Any]:
        ...
        # Call search_supplier_inventory
        search_response = mcp_client.call_tool("search_supplier_inventory", {
            "part_name": part_name
        })
        ...
        # Priority: 1) Lowest lead time (Emergency resolution), 2) Highest rating
        best_match = None
        for option in results:
            self.log(
                logs,
                f"SUPPLIER OPTION: '{option['supplier_name']}' has {option['stock']}x in stock. "
                f"Price: ${option['price']} USD. Lead time: {option['lead_time_hours']} hours. Rating: {option['rating']}."
            )
            
            if best_match is None:
                best_match = option
            else:
                # Compare lead time first
                if option["lead_time_hours"] < best_match["lead_time_hours"]:
                    best_match = option
                # If equal lead time, select higher rating
                elif option["lead_time_hours"] == best_match["lead_time_hours"]:
                    if option["rating"] > best_match["rating"]:
                        best_match = option

        self.log(
            logs,
            f"SUPPLIER SELECTED: Recommending '{best_match['supplier_name']}' for '{part_name}'. "
            f"Lead Time: {best_match['lead_time_hours']} hrs. Unit Price: ${best_match['price']} USD."
        )
